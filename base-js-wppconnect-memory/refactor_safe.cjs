...
if (content.includes('procesarEnvioMensaje')) {
...
const innerLogicMatch = apiBlock.match(/colaMensajesGlobal\.agregar\(async \(\) => \{[\s\S]*? \}\)[\s]*\}\)/);
...
// Ensure we don't grab handleCtx's closing '})'
let innerLogic = innerLogicMatch[0];
// Clean up the trailing handleCtx braces from innerLogic, it ends in "}\n            })\n        })" but should only be "}\n            })"
innerLogic = innerLogic.replace(/\s*\}\)\s*\}\)\s*$/, '\n            })');
